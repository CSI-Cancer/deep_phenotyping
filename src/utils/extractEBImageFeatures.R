...
image_folder_path <- commandArgs(trailingOnly = TRUE)[1]

# Load the images and the masks
image_names <- list.files(image_folder_path, full.names=TRUE, pattern="*.tif")
images <- lapply(image_names, readImage)
...
features = lapply(images, get_features)
...
features$image_name = image_names
print(dim(features))

# Save the features
write.table(features,file="features.txt",row.names=FALSE,quote=FALSE,sep="\t")